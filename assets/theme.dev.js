...
(function (scrollLock, Flickity, themeCurrency, Ajaxinate) {
  ...
  const initTransparentHeader = () => {
    ...
    const headerTransparent =
      header.getAttribute(attributes$U.transparent) === "true";
    ...
    const preventTransparentHeader = firstSection.querySelector(
      `${selectors$1g.preventTransparentHeader}:first-of-type`
    );
    window.isHeaderTransparent =
      headerTransparent &&
      firstSection.classList.contains(classes$14.supportsTransparentHeader) &&
      !preventTransparentHeader;

    const supportsHasSelector = CSS.supports("(selector(:has(*)))");
    if (!supportsHasSelector) {
      body.classList.toggle(
        classes$14.isFirstSectionTransparent,
        window.isHeaderTransparent
      );
      header.classList.toggle(
        classes$14.siteHeaderTransparent,
        window.isHeaderTransparent
      );
    }
  };
  ...
  const removeFooterWave = (section) => {
    ...
    const shadowSections = document.querySelectorAll(
      selectors$1f.sectionsShadow
    );
    ...
    const footerParallax = document.querySelectorAll(
      selectors$1f.footerParallax
    );
    if (footerParallax.length > 0) {
      const footerParallaxInner = footerParallax[0].querySelector(
        selectors$1f.footerParallaxInner
      );
  ...
  const reloadCardScrolling = () => {
    setTimeout(() => {
      ...
      const cardScrollingInstances = shopifyInstances.filter((instance) => {
        ...
        const hasCardScrollingEffect = instance.container.matches(
          selectors$1e.cardScrolling
        );

        const isEligible =
          (isSliderSection && hasCardScrollingEffect) ||
          (isBannerImageSection && hasCardScrollingEffect);
  ...
  function readHeights() {
    ...
    h.headerInitialHeight =
      parseInt(
        document.querySelector(selectors$1d.header)?.dataset.height ||
          document.querySelector(selectors$1d.header)?.offsetHeight
      ) || 0;
  ...
  function calcVars(checkOrientation = false) {
    ...
    const hasCollectionFilters = document.querySelector(
      selectors$1d.collectionFilters
    );
    const hasLogoTextLink =
      document.querySelector(selectors$1d.logoTextLink) !== null;
  ...
  function checkFirstSectionTransparency() {
    ...
    return firstSection.classList.contains(
      classes$12.supportsTransparentHeader
    );
  }
  ...
  function preventOverflow(container) {
    const singleFrames = container.querySelectorAll(
      selectors$1c.overflowContainer
    );
    if (singleFrames) {
  ...
  function removeScrollLock() {
    const isPopupVisible =
      document.body.classList.contains(classes$11.quickViewVisible) ||
      document.body.classList.contains(classes$11.cartDrawerOpen);
    ...
    document.addEventListener(
      ...
      (e) => {
        if (
          ...
          e.target.classList.contains(classes$10.loading)
        ) {
  ...
  class DeferredLoading extends HTMLElement {
    constructor() {
      ...
      if (this.hasAttribute(attributes$R.deferredContainer)) {
        this.container =
          this.closest(this.getAttribute(attributes$R.deferredContainer)) ||
          this.closest(selectors$1a.shopifySection);
      }
    ...
    loadTemplate() {
      ...
      const containsImages =
        deferredContent.querySelectorAll(selectors$1a.img).length > 0;
      if (containsImages) {
    ...
    reloadSrcset(container) {
      ...
      container.querySelectorAll(selectors$1a.img).forEach((img) => {
        const reloadSrcsetException = img.parentNode.matches(
          selectors$1a.reloadSrcsetException
        );
  ...
  class TextHighlight extends HTMLElement {
    ...
    constructor() {
      ...
      if (
        this.type === "circle-hand-drawn" ||
        this.type === "circle" ||
        this.type === "underline-hand-drawn"
      ) {
    ...
    connectedCallback() {
      ...
      this.animation = this.target.animate(
        settings$b[this.type].keyframes,
        settings$b[this.type].timing
      );
      ...
      if (parentAnimation) {
        ...
        if (this.textCountUp) {
          ...
          waitForAnimationEnd(parentAnimation).then(() => {
            const parentDelay =
              window
                .getComputedStyle(parentAnimation)
    ...
    shouldAnimate() {
      requestAnimationFrame(() => {
        ...
        if (inSlider)
          shouldAnimate = Boolean(this.closest(selectors$19.activeSlide));

        const stickyTextItem = this.closest(selectors$19.stickyTextItem);
        if (stickyTextItem)
          shouldAnimate = Boolean(
            stickyTextItem.classList.contains(classes$$.isActive)
          );
    ...
    onFinish() {
      ...
      if (this.hasAttribute(attributes$Q.isActive)) {
        const delay =
          this.getAttribute(attributes$Q.isActive) === "true"
            ? 0
            : settings$b[this.type].timing.delay;
    ...
    modifyDefaults(usage = "mouseenter", animation) {
      ...
      if (usage === "mouseleave") {
        ...
        if (
          this.type === "highlight" ||
          this.type === "highlight-color" ||
          this.type === "underline"
        ) {
          animation.effect.updateTiming({
            duration: settings$b[this.type].timing.duration,
          });
        }
        return;
      }
      ...
      if (usage === "slider") {
        if (!theme.settings.animationsEnabled) {
          ...
          animation.effect.updateTiming({
            delay: settings$b[this.type].timing.delay,
          });
        }
    ...
    bindAnimation(event = false) {
      ...
      if (event === "init")
        this.boundAnimation = this.bind.animate(
          settings$b[this.type].keyframes,
          settings$b[this.type].timing
        );
  ...
  class SearchForm extends HTMLElement {
    constructor() {
      ...
      this.popularSearches = this.popdownBody?.querySelector(
        selectors$18.popularSearches
      );
      this.predictiveSearchResults = this.querySelector(
        selectors$18.predictiveSearchResults
      );
      ...
      if (this.isSearchPage) {
        this.mainInputType = document.querySelector(
          `${selectors$18.mainInputSearch} ${selectors$18.inputType}`
        );
    ...
    onFormSubmit(event) {
      if (
        ...
        this.querySelector(selectors$18.selectedLink)
      )
    ...
    switchOption(direction) {
      ...
      const predictiveSearchOpened =
        this.classList.contains(classes$_.isSearched) &&
      ...
      this.selectedElement = visibleElementsContainer.querySelector(
        selectors$18.ariaSelected
      );

      // Filter out hidden elements
      const allVisibleElements = Array.from(
        visibleElementsContainer.querySelectorAll(
          selectors$18.allVisibleElements
        )
    ...
    handleFocusableDescendants(reset = false) {
      const selected = this.selectedElement
        ...
        : this.querySelector(selectors$18.ariaSelected);
  ...
  class PredictiveSearch extends SearchForm {
    constructor() {
      ...
      this.allPredictiveSearchInstances = document.querySelectorAll(
        selectors$17.predictiveSearch
      );
      this.predictiveSearchResults = this.querySelector(
        selectors$17.predictiveSearchResults
      );
      this.cachedResults = {};
    }
      ...
      fetch(
        ...
        .then((text) => {
          const resultsMarkup = new DOMParser()
            ...
            .querySelector(selectors$17.sectionPredictiveSearch).innerHTML;
    ...
    setLiveRegionLoadingState() {
      this.statusElement =
        this.statusElement ||
        this.querySelector(selectors$17.predictiveSearchStatus);
      this.loadingText =
        this.loadingText || this.getAttribute(attributes$P.loadingText);
    ...
    renderSearchResults(resultsMarkup) {
      ...
      const el = this.predictiveSearchResults.querySelector(
        selectors$17.searchResultsWrapper
      );
      const tempId = el.id;
    ...
    setLiveRegionResults() {
      ...
      this.setLiveRegionText(
        this.querySelector(selectors$17.searchResultsLiveRegion).textContent
      );
    }
  ...
  const selectors$16 = {
    ...
    watchMutations:
      '[id^="api-cart-items-"], [data-collection-filters], [data-collection-products], #AjaxinateLoop, [data-tab="resultsProducts"]',
  ...
  function onIntersecting(target) {
    ...
    if (target.hasAttribute(attributes$O.aosCarouselDesktop) && isDesktop()) {
      ...
      if (slider) {
        ...
        if (activeSlides.length > 0) {
          ...
          activeSlides[activeSlides.length - 1].setAttribute(
            attributes$O.aosTrigger,
            `#${triggerId}`
          );
          inactiveSlides.forEach((slide) => {
            slide
              .querySelectorAll(selectors$16.animatable)
              .forEach((element) => {
    ...
    if (target.hasAttribute(attributes$O.aosTrigger)) {
      ...
      const elementsToTrigger = document.querySelectorAll(
        `[${attributes$O.aosAnchor}="${triggerId}"]`
      );
    ...
    if (target.hasAttribute(attributes$O.aosWatchAnchors)) {
      ...
      const filteredElements = [...elementsToAnimate].filter((element) => {
        const anchor = element.hasAttribute(attributes$O.aosAnchor)
          ? element.getAttribute(attributes$O.aosAnchor)
          : false;
    ...
    elementsToAnimate.forEach((item) => {
      ...
      if (customInit) {
        target.dispatchEvent(
          new CustomEvent("theme:target:animate", {
            bubbles: true,
            detail: item,
          })
        );
    ...
    const bodyObserver = new MutationObserver((mutationList) => {
      for (const mutation of mutationList) {
        ...
        const watchMutations =
          ...
          addedNodes.some((item) =>
            isNode(item) ? item.matches(selectors$16.watchMutations) : false
          ) ||
          removedNodes.some((item) =>
            isNode(item) ? item.matches(selectors$16.watchMutations) : false
          );
        const isAnimatableElement =
          target.matches(selectors$16.animatable) ||
          addedNodes.some((item) =>
            isNode(item) ? item.matches(selectors$16.animatable) : false
          );
        const hasElementToAnimate =
          target.querySelector(
            `[${attributes$O.aosAnchor}="#${target.id}"]`
          ) !== null;
      ...
      if (
        ...
        for (let val = divisions; val >= 1; val--) {
          ...
          newNum = symbols.reduce((num, { char, i }) => {
            return num.length <= i
              ? num // If we don't have enough numbers, don't insert the symbol.
  ...
  class Collapsible {
    constructor(container) {
      ...
      this.transitionOverride = this.container.hasAttribute(
        attributes$M.transitionOverride
      );
    ...
    collapsibleToggle(e) {
      ...
      const trigger = e.target.matches(selectors$15.trigger)
        ? e.target
        : e.target.closest(selectors$15.trigger);
  ...
  class QuantityCounter {
    ...
    init() {
      ...
      this.buttons = this.quantity.querySelectorAll(
        selectors$14.quantityButton
      );
      this.increaseButton = this.quantity.querySelector(
        selectors$14.quantityPlusButton
      );
    ...
    changeValueOnClick() {
      this.buttons.forEach((element) => {
        element.addEventListener("click", (event) => {
          ...
          const isDescrease =
            clickedElement.matches(selectors$14.quantityMinusButton) ||
            clickedElement.closest(selectors$14.quantityMinusButton);
          const isIncrease =
            clickedElement.matches(selectors$14.quantityPlusButton) ||
            clickedElement.closest(selectors$14.quantityPlusButton);
    ...
    constructor(container) {
      ...
      if (this.player) {
        ...
        this.enableBackground =
          this.player.getAttribute(attributes$L.dataEnableBackground) ===
          "true";
        this.disableSound =
          this.player.getAttribute(attributes$L.dataEnableSound) === "false";
        this.enableAutoplay =
          this.player.getAttribute(attributes$L.dataEnableAutoplay) !== "false";
        this.enableLoop =
          this.player.getAttribute(attributes$L.dataEnableLoop) !== "false";
  ...
  class LoadVideoYT {
      ...
      if (this.player) {
        ...
        if (this.videoType == "youtube") {
          this.checkPlayerVisibilityFlag =
            this.player.getAttribute(attributes$K.dataCheckPlayerVisibility) ===
            "true";
          this.playerID = this.player.querySelector(selectors$12.youtubeWrapper)
            ? this.player.querySelector(selectors$12.youtubeWrapper).id
            : this.player.id;
          ...
          this.container.addEventListener(
            ...
            function (e) {
              if (
                e.target.matches(selectors$12.videoWrapper) ||
                e.target.closest(selectors$12.videoWrapper)
              ) {
                const playerID = e.target.querySelector(
                  selectors$12.videoIframe
                ).id;
        ...
        events: {
          onReady: (event) => {
            ...
            const enableSound =
              document
                .querySelector(`#${id}`)
                .getAttribute(attributes$K.dataEnableSound) === "true";
          ...
          onStateChange: (event) => {
            ...
            if (event.data == 1) {
              ...
              event.target
                .getIframe()
                .parentElement.classList.add(classes$U.loaded);
            }
          },
        },
      };
    ...
    onUnload() {
      const playerID =
        "youtube-" + this.container.getAttribute(attributes$K.dataSectionId);
      if (!players[playerID]) {
        return;
      }
  ...
  class ProductVideo {
    constructor(container) {
      ...
      this.autoplayVideo =
        this.container.getAttribute(attributes$J.dataAutoplayVideo) === "true";
      this.players = {};
    ...
    loadContent(mediaContainer) {
      if (
        mediaContainer
          .querySelector(selectors$10.deferredMedia)
          .getAttribute(attributes$J.loaded)
      ) {
        return;
      }
    ...
    pauseOtherMedia(mediaId, container) {
      ...
      const otherMedia = container.querySelectorAll(
        `${selectors$10.productMediaWrapper}:not(${currentMedia})`
      );
    ...
    constructor(container) {
      ...
      if (this.scrollbar && this.scrollbarItems) {
        ...
        if (this.scrollbarItems.children.length) {
          this.calculateTrack(
            this.scrollbarItems.querySelector(selectors$$.current)
          );
        }
      }
    }
  ...
  class Tooltip {
    ...
    addPin(stopMouseEnter = false) {
      ...
      const tooltipTargetArrow = tooltipTarget.querySelector(
        selectors$_.tooltipArrow
      );

      if (
        tooltipTarget &&
        ((stopMouseEnter &&
          !this.tooltip.hasAttribute(attributes$I.tooltipStopMouseEnter)) ||
          !stopMouseEnter)
      ) {
        ...
        const onTooltipHiding = (event) => {
          ...
          if (
            ...
            requestAnimationFrame(
              () => (tooltipTarget.style.transform = "translate(0, -100%)")
            );
          }
    ...
    init() {
      ...
      if (this.slider.hasAttribute(selectors$Z.dataMinValue)) {
        defaultMinValue = parseFloat(
          this.slider.getAttribute(selectors$Z.dataMinValue)
        );
      }
      ...
      if (this.slider.hasAttribute(selectors$Z.dataMaxValue)) {
        defaultMaxValue = parseFloat(
          this.slider.getAttribute(selectors$Z.dataMaxValue)
        );
      }
      ...
      if (this.slider.getAttribute(selectors$Z.dataStep)) {
        this.step = Math.abs(
          parseFloat(this.slider.getAttribute(selectors$Z.dataStep))
        );
      }
  ...
  class ProductForm {
    constructor(element, product, options) {
      ...
      this.optionInputs = this._initInputs(
        selectors$Y.optionInput,
        options.onOptionChange
      );

      this.planInputs = this._initInputs(
        selectors$Y.planInput,
        options.onPlanChange
      );

      this.quantityInputs = this._initInputs(
        selectors$Y.quantityInput,
        options.onQuantityChange
      );

      this.propertyInputs = this._initInputs(
        selectors$Y.propertyInput,
  ...
  class Swatch {
    ...
    setStyles() {
      const isNativeSwatch =
        window.theme.settings.colorSwatchesType == "native";
    ...
    replaceImages(id) {
      ...
      if (this.hoverImages.length > 1) {
        this.hoverImages.forEach((hoverImage) => {
          ...
          if (
            hoverImage.getAttribute(attributes$H.variantId) === this.variant
          ) {
  ...
  class SwatchesContainer {
    ...
    checkSwatchesHeight(swatchesContainer) {
      ...
      const labelMargin = hiddenLabels
        ? 0
        : parseInt(
      ...
      const selectorWrapper = swatchesContainer.closest(
        selectors$X.selectorWrapper
      );

      selectorWrapper.classList.remove(
        classes$P.selectorLarge,
        classes$P.selectorVisible
      );
      ...
      requestAnimationFrame(() => {
        if (
          ...
          labelHeight + labelMargin + swatch.offsetHeight * 2 + swatchMargin * 2
        ) {
  ...
  class ProductGrid {
    ...
    productGridSlideshow() {
      const productMediaSlider = this.container.querySelectorAll(
        selectors$W.productMediaSlider
      );
      const linkedImages = this.container.querySelectorAll(
        selectors$W.productMediaContainer
      );
      ...
      if (productMediaSlider.length) {
        productMediaSlider.forEach((slider) => {
          ...
          const progressBar = mediaContainer.querySelector(
            selectors$W.progressBar
          );
          const countImages = slider.querySelectorAll(
            selectors$W.productMediaSlide
          ).length;
  ...
  class Ajaxify {
    ...
    init() {
      ...
      this.ajaxinateContainer = this.container.querySelectorAll(
        selectors$V.ajaxinateContainer
      );
      ...
      this.ajaxinateContainer.forEach((element) => {
        ...
        if (hasChildren) {
          ...
          const columnsTablet = Number(
            element.getAttribute(attributes$G.columnsTablet)
          );
          const columnsMobile = Number(
            element.getAttribute(attributes$G.columnsMobile)
          );
          ...
          if (columnsTablet >= 3) {
            ...
            promos.forEach((promo) => {
              if (promo.classList.contains(classes$N.promoFull)) {
                if (promo.classList.contains(classes$N.promoGridThird)) {
                  ...
                  promo.classList.contains(classes$N.promoGridSecond)
                ) {
                  tabletCounter += 2;
                } else {
                  tabletCounter += 3;
            ...
            if (tabletCounter > 0) {
              let emptySpacesTablet =
                columnsTablet - (tabletCounter % columnsTablet);
              if (emptySpacesTablet === columnsTablet) {
                emptySpacesTablet = 0;
              }
              element.classList.add(
                `grid--empty-items-tablet-${emptySpacesTablet}`
              );
            }
          }
          ...
          if (columnsMobile === 2) {
            ...
            promos.forEach((promo) => {
              if (
                promo.classList.contains(classes$N.promoGridEven) ||
                promo.classList.contains(classes$N.promoOneColumn)
              ) {
                mobileCounter += 1;
              }
            });
            ...
            if (mobileCounter > 0) {
              let emptySpacesMobile =
                columnsMobile - (mobileCounter % columnsMobile);
              if (emptySpacesMobile === columnsMobile) {
                emptySpacesMobile = 0;
              }
              element.classList.add(
                `grid--empty-items-mobile-${emptySpacesMobile}`
              );
            }
          }
    ...
    update(id) {
      ...
      const elementIDMatch = (element) =>
        `${selectors$V.ajaxinateContainer}[${attributes$G.ajaxinateId}="${element.dataset.ajaxinateId}"]` ===
        id;
    ...
    loadMoreFix() {
      ...
      Ajaxinate.prototype.loadMore = function loadMore() {
        ...
        this.request.onreadystatechange = function success() {
          ...
          const newProducts = newContainer.querySelectorAll(
            selectors$V.productItem
          );
          newProducts.forEach((item) =>
            item.classList.add(classes$N.loadedItem)
          );
    ...
    unload() {
      if (this.endlessScroll.length > 0) {
        ...
        this.ajaxinateContainer.forEach((element) =>
          element.classList.remove(classes$N.isLoaded)
        );
      }
    }
  }
  ...
  class Filters {
    constructor(container) {
      ...
      this.filterTriggers = this.container.querySelectorAll(
        selectors$U.collapsibleTrigger
      );
      this.filtersStickyBar = this.container.querySelector(
        selectors$U.filtersStickyBar
      );
      ...
      this.collectionSortOptions = this.container.querySelector(
        selectors$U.collectionSortOptions
      );
      this.a11y = a11y;
      this.filterData = [];
    ...
    updateRange() {
      ...
      if (
        rangeMin.hasAttribute(selectors$U.rangeMinValue) &&
        rangeMax.hasAttribute(selectors$U.rangeMaxValue)
      ) {
        ...
        const rangeMinValue = parseFloat(
          rangeMin.getAttribute(selectors$U.rangeMinValue),
          10
        );
        const rangeMaxValue = parseFloat(
          rangeMax.getAttribute(selectors$U.rangeMaxValue),
          10
        );
    ...
    onSubmitHandler(event) {
      ...
      if (
        ...
        const rangeMinDefault = parseFloat(
          rangeMin.getAttribute(selectors$U.rangeMinDefault),
          10
        );
        const rangeMaxDefault = parseFloat(
          rangeMax.getAttribute(selectors$U.rangeMaxDefault),
          10
        );
    ...
    renderProductGrid(html) {
      const newProductGrid = new DOMParser()
        ...
        .querySelector(selectors$U.productGrid);
    ...
    updateProductsCount(html) {
      const newProductsCount = new DOMParser()
        .parseFromString(html, "text/html")
        .querySelector(selectors$U.productsCount);
    ...
    renderFilters(html) {
      const newFilters = new DOMParser()
        .parseFromString(html, "text/html")
        .querySelector(selectors$U.filters);
    ...
    bindFilterButtonsEvents() {
      ...
      this.container
        .querySelectorAll(selectors$U.filterResetButton)
        .forEach((button) => {
    ...
    bindToggleButtonsEvents() {
      this.container
        .querySelectorAll(selectors$U.toggleFilters)
        .forEach((button) => {
          button.addEventListener("click", this.onFilterToggleClick);
        });
      this.container
        .querySelectorAll(selectors$U.closeFilters)
        .forEach((button) => {
          button.addEventListener("click", this.hideFiltersDrawer);
        });
      this.container
        .querySelectorAll(selectors$U.openFilters)
        .forEach((button) => {
          button.addEventListener("click", this.showFiltersDrawer);
        });

      this.container
        .querySelector(selectors$U.collectionWrapper)
    ...
    onTabHandler(event) {
      if (
        ...
        this.filtersForm
          .querySelectorAll(selectors$U.inputSort)
          .forEach((input) => {
            if (input.checked) {
              input.checked = false;
            }
    ...
    onFilterToggleClick(event) {
      ...
      const filtersVisible = this.filters.classList.contains(
        classes$M.collectionFiltersVisible
      );
    ...
    sortDropdownToggle() {
      ...
      this.collectionSortOptions.classList.toggle(
        classes$M.collectionSortOptionWrapperVisible
      );
    }
    ...
    mobileFiltersScrollLock() {
      ...
      if (window.innerWidth < theme.sizes.small) {
        const scrollableElement = document.querySelector(
          selectors$U.filtersList
        );

        if (
          !this.filters.classList.contains(classes$M.collectionFiltersVisible)
        ) {
    ...
    hideFiltersOnMobile() {
      const filterTriggers = this.container.querySelectorAll(
        `${selectors$U.collapsibleTrigger}:not(${selectors$U.showMoreTrigger})`
      );
        ...
        requestAnimationFrame(() => {
          filterTriggers.forEach((element) => {
            const isFilterActive =
              element.getAttribute(attributes$F.filterActive) === "true";

            if (
              element.classList.contains(classes$M.isExpanded) &&
              !isFilterActive
            ) {
    ...
    showFiltersOnDesktop() {
      ...
      const filtersDefaultState = this.container.getAttribute(
        attributes$F.filtersDefaultState
      );
      ...
      if (!loading) {
        document.dispatchEvent(
          new CustomEvent("theme:scroll:unlock", {
            bubbles: true,
            detail: settings$8.loadingTimeout,
          })
        );
      }
    }
    ...
    updateCollapsedContainerFocusableElements(
      ...
      requestAnimationFrame(() => {
        const isFilterExpanded =
          filterCollapsibleTrigger.getAttribute(attributes$F.ariaExpanded) ===
          "true";
        const isShowMoreExpanded =
          showMoreTrigger.getAttribute(attributes$F.ariaExpanded) === "true";
    ...
    updateShowMoreFocusableElements(event, focusable) {
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          const isExpanded =
            event.target.getAttribute(attributes$F.ariaExpanded) === "true";
    ...
    bindFilterTagButtonsEvents() {
      this.container
        .querySelectorAll(selectors$U.filterTagButton)
        .forEach((button) => {
      ...
      this.container
        .querySelectorAll(selectors$U.filterTagClearButton)
        .forEach((button) => {
    ...
    onFilterTagButtonClick(event) {
      ...
      this.container
        .querySelector(selectors$U.filter)
        .classList.remove(classes$M.isExpanded);
      this.container
        .querySelector(selectors$U.filter)
        .setAttribute(attributes$F.ariaExpanded, false);
    ...
    onFilterTagResetClick(event) {
      ...
      this.container
        .querySelectorAll(selectors$U.filterTag)
        .forEach((element) => {
    ...
    finishLoading() {
      const popups = document.querySelectorAll(
        `${selectors$U.popupsSection} .${classes$M.popupVisible}`
      );
  ...
  class Tabs {
    constructor(container) {
      ...
      this.isSearchPage =
        container.closest(`[${attributes$E.searchPerformed}="true"]`) != null;
      ...
      if (this.container) {
        ...
        this.inactiveTabsAnimationsCallback = debounce(
    ...
    assignSearchPageArguments() {
      ...
      this.searchForAllTypes =
        this.container.getAttribute(attributes$E.allTypes) === "true";
      this.fetchURL = "";
      this.searchParams = "";
      this.cachedResults = {};
    ...
    init() {
      ...
      const firstTabsLink = this.container.querySelector(
        `[${attributes$E.tabsLink}="${
          this.container.hasAttribute(attributes$E.tabStartIndex)
            ? this.container.getAttribute(attributes$E.tabStartIndex)
            : 0
        }"]`
      );
    ...
    handleURLSearchParams(event, updateHistory = true) {
      const target = event.target.matches(selectors$T.tabsLink)
        ? event.target
        : event.target.closest(selectors$T.tabsLink);
      ...
      let currentPageStr = currentPage
        ? `&page=${currentPage.getAttribute(attributes$E.currentPage)}`
        : "";
      ...
      fetch(this.fetchURL)
        ...
        .then((text) => {
          ...
          if (this.searchForAllTypes) {
            this.container
              .querySelector(selectors$T.allTypesContainer)
              ?.remove();
          }
    ...
    onHistoryChange(event) {
      ...
      const typeProduct = this.container.querySelector(
        `${selectors$T.tabsLink}[${attributes$E.type}="product"]`
      );
      const typeArticle = this.container.querySelector(
        `${selectors$T.tabsLink}[${attributes$E.type}="article"]`
      );
      const typePage = this.container.querySelector(
        `${selectors$T.tabsLink}[${attributes$E.type}="page"]`
      );
    ...
    initInfinityScroll(type) {
      ...
      const ajaxinateContainer = this.container.querySelectorAll(
        selectors$T.ajaxinateContainer
      );
      ...
      const activeTab = this.container.querySelector(
        `${selectors$T.tab}.${classes$L.current}`
      );
      ...
      setTimeout(() => {
        ...
        [...this.endlessCollection.endlessScroll].forEach((instance) => {
          ...
          const activeTabPresent = [...this.tab].find((tab) =>
            tab.classList.contains(classes$L.current)
          );
          const isInActiveTab =
    ...
    handleActiveTabClasses(element, tab) {
      ...
      const lastActiveTabsLink = this.container.querySelector(
        `${selectors$T.tabsLink}.${classes$L.current}`
      );
      ...
      this.tabRef?.forEach((refElement) => {
        ...
        const shouldBeActive =
          refElement.getAttribute(attributes$E.tabRef) ===
          tab.getAttribute(attributes$E.tab);
    ...
    scrollToCurrentTabLink(element) {
      const parent = element.closest(selectors$T.scrollableHolder)
        ? element.closest(selectors$T.scrollableHolder)
    ...
    triggerTabAnimations(tab) {
      ...
      const productTab = this.tabsContents.querySelector(
        `[${attributes$E.tab}="resultsProducts"]`
      );

      // Product tab is current
      if (productTab && productTab.classList.contains(classes$L.current)) {
        const anchors = this.tabsContents.querySelectorAll(
          selectors$T.aosAnchor
        );
        // Get all anchors and attach observers
        initAnchorObservers(anchors);
        return;
      ...
      this.tabsContents
        .querySelectorAll(selectors$T.aosItem)
        .forEach((element) => {
    ...
    checkVisibleTabsLinks() {
      ...
      const tabsNavListHidden = this.container.querySelectorAll(
        `${selectors$T.tabsLink}.${classes$L.hide}`
      );
  ...
  class Drawer {
    constructor(container) {
      ...
      this.drawerCloseEvent = (event) => {
        const activeDrawer = document.querySelector(
          `${selectors$S.drawer}.${classes$K.open}`
        );
    ...
    toggle(e) {
      ...
      const drawer = document.querySelector(
        `#${e.target.getAttribute(attributes$D.ariaControls)}`
      );
      if (!drawer) {
        return;
      }
    ...
    open(e) {
      ...
      const drawerScroller =
        drawer.querySelector(selectors$S.scroller) || drawer;
  ...
  class StoreAvailability {
      ...
      fetch(variantSectionUrl)
        ...
        .then(function (storeAvailabilityHTML) {
          ...
          let storeAvailabilityModal = body.querySelector(
            selectors$Q.storeAvailabilityModal
          );
          ...
          const storeAvailabilityModalOpen = self.container.querySelector(
            selectors$Q.storeAvailabilityModalOpen
          );
    ...
    _updateProductTitle(productTitle) {
      const storeAvailabilityModalProductTitle = this.container.querySelector(
        selectors$Q.storeAvailabilityModalProductTitle
      );
  ...
  class NotificationPopup {
    constructor(button) {
      ...
      this.notificationPopupHtml = this.button.getAttribute(
        attributes$B.notificationPopup
      );
  ...
  class ProductAddForm {
    constructor(container) {
      ...
      this.tallLayout =
        this.container.getAttribute(attributes$A.tallLayout) === "true";
      this.addToCartButton = this.container.querySelector(
        selectors$O.addToCart
      );
      this.variantOptionImages = this.container.querySelectorAll(
        selectors$O.variantOptionImage
      );
    ...
    init() {
      ...
      if (this.hasVariantOptionWithImage) {
        this.variantButtonsContainer = this.variantOptionImages[0].closest(
          selectors$O.variantButtons
        );
    ...
    linkForm() {
      ...
      if (this.swatchesContainer) {
        ...
        const selectorWrapper = this.swatchesContainer.closest(
          selectors$O.selectorWrapper
        );
        const moreLink = selectorWrapper.querySelector(
          selectors$O.swatchesMore
        );
    ...
    updateAddToCartState(formState) {
      ...
      formWrapper?.forEach((element) => {
        ...
        const formSelect = element.querySelector(
          selectors$O.originalSelectorId
        );
        if (formSelect) {
    ...
    updateRemaining(formState) {
      ...
      const remainingClasses = [
        classes$G.remainingIn,
        classes$G.remainingOut,
        classes$G.remainingUnavailable,
        classes$G.remainingLow,
      ];
    ...
    equalizeImageOptionContainers() {
      ...
      const heights = [...this.variantOptionImages].map((item) =>
        Math.floor(item.offsetHeight)
      );
      const widths = [...this.variantOptionImages].map((item) =>
        Math.floor(item.offsetWidth)
      );
      const widest = Math.max(...widths);
      const tallest = Math.max(...heights);

      this.variantButtonsContainer.style.setProperty(
        "--option-image-width",
        widest + "px"
      );
      this.variantButtonsContainer.style.setProperty(
        ...
        toggle.addEventListener(
          ...
          function (e) {
            ...
            const selected = this.container.querySelector(
              `[${selectors$O.subsChild}="${val}"]`
            );
            const groups = this.container.querySelectorAll(
              `[${selectors$O.subsChild}]`
            );
            if (selected) {
    ...
    updateProductUnits(formState) {
      ...
      if (
        ...
        (!plan && !variant.requires_selling_plan)
      ) {
      ...
      if (unitPrice) {
        ...
        this.container.querySelector(selectors$O.unitPrice).innerHTML =
          formattedPrice;
    ...
    updateProductImage(evt) {
      ...
      const newImg = this.container.querySelector(
        `${selectors$O.productImage}[${attributes$A.productImageId}="${variant.featured_media.id}"]`
      );
    ...
    observeSwatch(formState) {
      const swatch = this.swatchesContainer.querySelector(
        `[${attributes$A.swatchVariant}*="${formState.variant.id}"]`
      );
      this.swatchesContainer
        .closest(selectors$O.selectorWrapper)
        .classList.remove(classes$G.selectorVisible);
        ...
        (entries, observer) => {
          entries.forEach((entry) => {
            ...
            if (notVisible) {
              this.swatchesContainer
                .closest(selectors$O.selectorWrapper)
                .classList.add(classes$G.selectorVisible);
            }
          });
        },
        {
    ...
    scrollToForm(section) {
      const headerHeight = document.querySelector(selectors$O.header)?.dataset
        .height;
      const isVisible =
    ...
    onUnload() {
      ...
      if (this.hasVariantOptionWithImage)
        window.removeEventListener("theme:resize:width", this.onResizeCallback);
    }
  }
  ...
  class Popout {
    constructor(popout) {
      ...
      this.popoutOptions = this.popout.querySelectorAll(
        selectors$N.popoutOptions
      );
      this.popoutPrevent =
        this.popout.getAttribute(attributes$z.popoutPrevent) === "true";
    ...
    popupToggleClick(evt) {
      const ariaExpanded =
        evt.currentTarget.getAttribute(attributes$z.ariaExpanded) === "true";

      if (this.popoutList.closest(selectors$N.productGrid)) {
        const productGridItemImage = this.popoutList
          .closest(selectors$N.productGrid)
          .querySelector(selectors$N.productGridImage);
    ...
    switchQuantityInputBehavior(prevent = true) {
      if (
        prevent ||
        !this.popoutInput.hasAttribute(attributes$z.popoutQuantity)
      )
        return;

      const targetElement = this.popoutList.querySelector(
        `[${attributes$z.dataValue}="${this.popoutInput.value}"]`
      );
  ...
  class LoadQuickview {
    constructor(popup, pswpElement) {
      ...
      this.quickViewFoot = this.pswpElement.querySelector(
        selectors$M.quickViewFootInner
      );
      this.quickViewInner = this.pswpElement.querySelector(
        selectors$M.quickViewInner
      );
      ...
      this.product.forEach((item, index) => {
        const isQuickViewOnboarding = item.hasAttribute(
          attributes$y.quickViewOnboarding
        );
    ...
    initShopTheLookListeners() {
      this.buttonsShopTheLookThumb?.forEach((button) => {
        button.addEventListener("click", (event) => {
          ...
          const thumb = event.target.matches(selectors$M.shopTheLookThumb)
            ? event.target
            : event.target.closest(selectors$M.shopTheLookThumb);
          const holder = this.pswpElement.querySelector(
            `[${attributes$y.hotspot}="${thumb.getAttribute(
              attributes$y.hotspotRef
            )}"]`
          );
            ...
            requestAnimationFrame(() => {
              this.flkty.forEach((slider) => {
                ...
                const allMediaItems = this.quickViewInner.querySelectorAll(
                  selectors$M.productMediaWrapper
                );
    ...
    prevent3dModelSubmit(event) {
      if (
        event.submitter.closest(selectors$M.deferredMedia) &&
        event.submitter.closest(selectors$M.productForm)
      ) {
        event.preventDefault();
      }
    }
    ...
    closeQuickviewOnMobile() {
      if (
        ...
        document.body.classList.contains(classes$E.jsQuickViewVisible)
      ) {
        this.popup.close();
      }
    }
    ...
    closeOnAnimationEnd(event) {
      if (
        ...
        this.popup.template.classList.remove(
          classes$E.popupQuickViewAnimateOut,
          classes$E.popupQuickViewAnimated
        );
        this.popup.close();
    ...
    closeOnEscape(event) {
      ...
      const isNotificationVisible = document.body.classList.contains(
        classes$E.notificationPopupVisible
      );
        ...
        if (mediaItems.length) {
          mediaItems.forEach((element) => {
            element.addEventListener("theme:media:play", () => {
              ...
              element
                .closest(selectors$M.productMediaSlider)
                .classList.add(classes$E.hasMediaActive);
            });
            ...
            element.addEventListener("theme:media:pause", () => {
              ...
              element
                .closest(selectors$M.productMediaSlider)
                .classList.remove(classes$E.hasMediaActive);
            });
          });
        }
    ...
    switchMedia(item, mediaId) {
      ...
      const selectedMedia = item.querySelector(
        `${selectors$M.productMediaWrapper}[${attributes$y.mediaId}="${mediaId}"]`
      );
      const isFocusEnabled = !document.body.classList.contains(
        classes$E.noOutline
      );
      ...
      if (
        deferredMedia &&
        deferredMedia.getAttribute(attributes$y.loaded) !== "true"
      ) {
        selectedMedia
    ...
    initShopifyXrLaunch(item) {
      document.addEventListener("shopify_xr_launch", () => {
        const currentMedia = item.querySelector(
          `${selectors$M.productModel}:not(.${classes$E.mediaHidden})`
        );
    ...
    addFormSuffix(item) {
      ...
      const productObject = JSON.parse(
        item.querySelector(selectors$M.productJSON).innerHTML
      );
  ...
  class LoadPhotoswipe {
    ...
    initLoader() {
      if (
        this.pswpElement.classList.contains(classes$D.customLoader) &&
        this.options !== "" &&
        this.options.mainClass
      ) {
        this.pswpElement.setAttribute(
          attributes$x.dataOptionClasses,
          this.options.mainClass
        );
      ...
      if (this.popupThumbsContainer && this.popupThumbsContainer.firstChild) {
        ...
        this.popupThumbs.forEach((element, i) => {
          element.addEventListener("click", (e) => {
            e.preventDefault();
            element.parentElement
              .querySelector(`.${classes$D.current}`)
              .classList.remove(classes$D.current);
    ...
    setCurrentThumb() {
      ...
      const lastCurrentThumb = this.pswpElement.querySelector(
        `${selectors$L.popupThumbs} > .${classes$D.current}`
      );
      if (lastCurrentThumb) {
  ...
  class QuickViewPopup {
    constructor(container) {
      ...
      this.buttonsQuickView = this.container.querySelectorAll(
        selectors$K.buttonQuickView
      );
      this.buttonsShopTheLookQuickView = this.container.querySelectorAll(
        selectors$K.shopTheLookQuickViewButton
      );
    ...
    popupInit(trigger) {
      ...
      const holder = this.loadPhotoswipe.pswpElement.querySelector(
        `[${attributes$w.hotspot}="${trigger.getAttribute(
          attributes$w.hotspot
        )}"]`
      );
      const quickViewItemHolders =
        this.loadPhotoswipe.pswpElement.querySelectorAll(
          selectors$K.quickViewItemHolder
        );
    ...
    initPhotoswipe(event) {
      ...
      const button = event.target.matches(selectors$K.buttonQuickView)
        ? event.target
        : event.target.closest(selectors$K.buttonQuickView);
      const isMobile = window.innerWidth < theme.sizes.small;
      ...
      if (isShopTheLookPopupTrigger) {
        ...
        const quickViewItemsTemplate = this.container
          .querySelector(selectors$K.quickViewItemsTemplate)
          .content.firstElementChild.cloneNode(true);
  ...
  class CartDrawer {
    ...
    init() {
      ...
      this.recipientErrors =
        this.form?.getAttribute(attributes$v.recipientError) === "true";
    ...
    assignArguments() {
      ...
      this.emptyMessageBottom = document.querySelector(
        selectors$J.emptyMessageBottom
      );
      ...
      if (this.cartMessage.length > 0) {
        this.cartFreeLimitShipping =
          Number(
            this.cartMessage[0].getAttribute(attributes$v.shippingMessageLimit)
          ) *
          100 *
    ...
    cartAddEvent() {
      document.addEventListener("click", (event) => {
        ...
        if (isButtonATC || getButtonATC) {
          ...
          const hasInputsInNoScript = [...this.form.elements].some((el) =>
            el.closest(selectors$J.noscript)
          );
    ...
    updateCart(updateData = {}, currentItem = null) {
      ...
      const newItem =
        this.cart.querySelector(`[${attributes$v.item}="${updateData.id}"]`) ||
        currentItem;
      const lineIndex = newItem?.hasAttribute(attributes$v.itemIndex)
        ? parseInt(newItem.getAttribute(attributes$v.itemIndex))
        : 0;
      const itemTitle = newItem?.hasAttribute(attributes$v.itemTitle)
        ? newItem.getAttribute(attributes$v.itemTitle)
        : null;
    ...
    disableCartButtons() {
      ...
      const buttons = this.cart.querySelectorAll(
        `button, ${selectors$J.cartItemRemove}`
      );
    ...
    updateErrorText(itemTitle) {
      this.cartErrorHolder.querySelector(selectors$J.errorMessage).innerText =
        itemTitle;
    }
    ...
    addToCartError(data) {
      ...
      let holder = this.button.closest(selectors$J.productForm)
        ? this.button.closest(selectors$J.productForm)
        : this.button.closest(selectors$J.upsellHolder);
      let errorContainer = holder.querySelector(
        selectors$J.formErrorsContainer
      );
      ...
      if (
        this.cartDrawerEnabled &&
        this.button &&
        this.button.closest(selectors$J.cartDrawer) !== null &&
        !this.button.closest(selectors$J.cartDrawer)
      ) {
    ...
    handleCloseErrorMessages(container) {
      const formErrorClose = container.querySelector(
        selectors$J.formCloseError
      );

      formErrorClose?.addEventListener("click", (event) => {
        const clickedElement = event.target;
        const isFormCloseError =
          clickedElement.matches(selectors$J.formCloseError) ||
          clickedElement.closest(selectors$J.formCloseError);
    ...
    hideAddToCartErrorMessage() {
      const holder = this.button.closest(selectors$J.upsellHolder)
        ? this.button.closest(selectors$J.upsellHolder)
        : this.button.closest(selectors$J.productForm);
    ...
    renderCartDrawer(alwaysOpen = true) {
      const cartDrawerTemplate = document.querySelector(
        selectors$J.cartDrawerTemplate
      );
    ...
    animateCartItems(state = "in") {
      ...
      if (state === "in") {
        items.forEach((item) =>
          requestAnimationFrame(() => item.classList.add(classes$B.aosAnimate))
        );
      }
    ...
    cartDrawerToggleEvents() {
      ...
      this.cartDrawerCloseEvent = (event) => {
        ...
        const isCartDrawerChild = document
          .querySelector(selectors$J.cartDrawer)
          .contains(event.target);
        const isPopupQuickView = event.target.closest(
          selectors$J.popupQuickView
        );
    ...
    toggleClassesOnContainers() {
      ...
      if (this.isCartPage) {
        ...
        this.cartForm.classList.toggle(
          classes$B.hidden,
          !that.hasItemsInCart()
        );
        this.emptyMessageBottom.classList.toggle(
          classes$B.hidden,
          that.hasItemsInCart()
        );
        this.cartNoteHolder.classList.toggle(
          classes$B.hidden,
          !that.hasItemsInCart()
        );
      }
    ...
    build(data) {
      ...
      this.newTotalItems =
        cartItemsData && cartItemsData.querySelectorAll(selectors$J.item).length
          ? cartItemsData.querySelectorAll(selectors$J.item).length
          : 0;
      this.subtotal =
        cartTotal && cartTotal.hasAttribute(attributes$v.cartTotal)
    ...
    getCartItemCount() {
      ...
      return Array.from(
        this.cart.querySelectorAll(selectors$J.qtyInput)
      ).reduce(
    ...
    freeShippingMessageHandle(total) {
      if (this.cartMessage.length > 0) {
        document
          .querySelectorAll(selectors$J.cartMessage)
          .forEach((message) => {
            const hasFreeShipping =
              message.hasAttribute(attributes$v.cartMessageValue) &&
              message.getAttribute(attributes$v.cartMessageValue) === "true" &&
              total !== 0;
            const cartMessageDefault = message.querySelector(
              selectors$J.cartMessageDefault
            );
        ...
        document
          ...
          .forEach((message) => {
            const cartMessageProgressItems = message.querySelectorAll(
              selectors$J.cartProgress
            );
            const leftToSpendMessage = message.querySelector(
              selectors$J.leftToSpend
            );
    ...
    observeAdditionalCheckoutButtons() {
      ...
      const additionalCheckoutButtons = this.cart.querySelector(
        selectors$J.additionalCheckoutButtons
      );
    ...
    scrollToCartTop() {
      ...
      window.scrollTo({
        top:
          this.outerSection?.offsetTop -
          this.headerInitialHeight -
  ...
  class SearchPopdown extends HTMLElement {
    constructor() {
      ...
      this.popularSearchesLink = this.querySelectorAll(
        selectors$H.popularSearchesLink
      );
      this.searchFormWrapper = this.querySelector(selectors$H.searchForm)
        ? this.querySelector(selectors$H.searchForm)
        : this.querySelector(selectors$H.predictiveSearch);
      ...
      this.mobileMenu = this.headerSection?.querySelector(
        selectors$H.mobileMenu
      );
      this.a11y = a11y;
      this.ensureClosingOnResizeEvent = () => this.ensureClosingOnResize();
      this.popdownTransitionCallbackEvent = (event) =>
        this.popdownTransitionCallback(event);
      this.detailsToggleCallbackEvent = (event) =>
        this.detailsToggleCallback(event);
    ...
    connectedCallback() {
      if (this.isPopdownInHeader) {
        this.searchFormInner.addEventListener(
          "transitionend",
          this.popdownTransitionCallbackEvent
        );
        ...
        this.details.addEventListener(
          "toggle",
          this.detailsToggleCallbackEvent
        );
        this.popdownClose.addEventListener("click", (event) =>
          this.close(event)
        );
    ...
    onPopdownToggleClick(event) {
      ...
      if (isChrome && !isIOS) {
        event.target
          .closest(selectors$H.details)
          .setAttribute(attributes$u.open, "");
      }
      if (
        event.target
          .closest(selectors$H.details)
          .hasAttribute(attributes$u.open)
      ) {
        event.preventDefault();
        this.close();
      }
    ...
    triggerPopdownOpen(event) {
      const isSearchPageWithoutTerms = this.closest(
        `[${attributes$u.searchPerformed}="false"]`
      );
      ...
      if (shouldOpenPopdownOnTouchDevice || shouldOpenPopdownOnNoTerms) {
        ...
        const isHeaderCompressed = this.header.classList.contains(
          classes$z.headerCompress
        );
        let popdownToggle = this.mobileMenu.querySelector(
          selectors$H.popdownToggle
        );

        if (!isTouch) {
          popdownToggle = isHeaderCompressed
            ? this.nav.querySelector(
                `${selectors$H.navItemsCompress} ${selectors$H.popdownToggle}`
              )
            : this.nav.querySelector(
                `${selectors$H.navIcons} ${selectors$H.popdownToggle}`
              );
        }
    ...
    open() {
      ...
      this.searchFormWrapper.input.setAttribute(
        attributes$u.ariaExpanded,
        true
      );
    ...
    close(event) {
      ...
      const ariaActivedescendant = this.searchFormWrapper.input.getAttribute(
        attributes$u.ariaActivedescendant
      );
      if (
        event &&
        event.target === this.popdownClose &&
        ...
        fetch(
          ...
          .then((product) => {
            ...
            productReplaced = productReplaced.includes("||itemAosDelay||")
              ...
              : productReplaced;
            productReplaced = productReplaced.includes("||itemAosAnchor||")
              ? productReplaced.replaceAll("||itemAosAnchor||", aosAnchor)
              : productReplaced;
  ...
  class Addresses {
    constructor(section) {
      ...
      this.addressNewForm = this.section.querySelector(
        selectors$G.addressNewForm
      );
      this.accountForms = this.section.querySelectorAll(
        selectors$G.accountForm
      );
      ...
      if (this.addressNewForm) {
        ...
        if (editButtons.length) {
          editButtons.forEach((button) => {
            button.addEventListener("click", function (e) {
              ...
              section
                .querySelector(`[${selectors$G.editAddress}="${formId}"]`)
                .classList.toggle(classes$y.hidden);
            });
          });
        }
        ...
        if (cancelButtons.length) {
          cancelButtons.forEach((button) => {
            button.addEventListener("click", function (e) {
              e.preventDefault();
              this.closest(selectors$G.accountForm).classList.add(
                classes$y.hidden
              );
              document
                .querySelector(selectors$G.btnNew)
                .classList.remove(classes$y.hidden);
            });
          });
        }
      }
    ...
    customerAddresses() {
      ...
      if (Shopify.CountryProvinceSelector) {
        new Shopify.CountryProvinceSelector(
          selectors$G.addressCountryNew,
          selectors$G.addressProvinceNew,
          {
      ...
      const countryOptions = this.section.querySelectorAll(
        selectors$G.addressCountryOption
      );
      ...
      if (
        ...
        if (this.scale.childElementCount === 1) {
          ...
          const animationTimeFrame =
            (this.text.clientWidth / settings$3.space) * speed;
  ...
  class ParallaxElement {
    constructor(el) {
      ...
      this.animation = this.container.getAttribute(
        attributes$r.parallaxAnimation
      );
      this.animations = this.animation.split(",");
      this.disableOnMobile = this.container.hasAttribute(
        attributes$r.disableOnMobile
      );
    ...
    assignFooterArguments() {
      ...
      if (
        this.prevSection &&
        !this.prevSection.classList.contains(classes$v.shopifySection) &&
        !this.prevSection.classList.contains(classes$v.mainContent)
      ) {
    ...
    assignCardScrollingArguments() {
      ...
      this.cardScrollingMarginTop = this.nextSection
        ? parseInt(
            window
              .getComputedStyle(this.nextSection)
      ...
      if (this.prevSection) {
        ...
        const matchesFirstSection = this.prevSection.matches(
          `#${firstSectionId}`
        );
        const prevSectionHeight = this.prevSection.offsetHeight;
        this.shopifySection.classList.toggle(
          classes$v.stickyTopZero,
          matchesFirstSection && prevSectionHeight <= this.headerHeight * 2
        );
      }
      ...
      this.opacityValue = (delta) => {
        ...
        if (Boolean(delta > 0 && delta <= 100))
          return Number(delta / 100).toFixed(4);
    ...
    updateParallax() {
      ...
      this.elementOffsetTopPoint = Math.round(
      ...
      this.elementOffsetBottomPoint =
        this.elementOffsetTopPoint + this.containerHeight;
      this.isBottomOfElementPassed =
        this.elementOffsetBottomPoint < this.scrollTop;
      this.isTopOfElementReached =
        this.elementOffsetTopPoint < this.scrollBottom;
      this.isInView =
        this.isTopOfElementReached && !this.isBottomOfElementPassed;
      ...
      if (this.isInView) {
        ...
        this.percentage =
          ((this.scrollProgress - this.containerHeight / 2) * 100) /
          this.viewportHeight; // -50% to 50% percent progress of the section
        this.percentageFull = Number(
          this.scrollProgress / this.containerHeight
        ).toFixed(2); // 0.00 to 1.00 - percent progress of the section
        ...
        if (
          this.animations.includes("horizontal") ||
          this.animations.includes("vertical") ||
          this.animations.includes("diagonal")
        ) {
          this.animateOverlappingImages();
        }
        if (
          this.animations.includes("offset-ltr") ||
          this.animations.includes("offset-rtl")
        ) {
          this.offsetX();
        }
    ...
    adjustCalculations() {
      if (this.animations.includes("card-scrolling")) {
        this.currentSectionTop =
          this.shopifySection.getBoundingClientRect().top;
      ...
      if (this.animations.includes("footer")) {
        this.elementOffsetTopPoint = Math.round(
          this.prevSection?.getBoundingClientRect().bottom -
            this.footerPrevSectionOffset +
            this.scrollTop
        );
        this.elementOffsetBottomPoint =
          this.elementOffsetTopPoint + this.containerHeight;
        ...
        this.isInView =
          this.isTopOfElementReached && !this.isBottomOfElementPassed;
        this.isApproaching =
          Math.round(this.elementOffsetTopPoint - this.viewportHeight / 2) <
          this.scrollBottom;
      }
    }
    ...
    offsetX() {
      ...
      const intensity =
        this.container.getAttribute(attributes$r.parallaxIntensity) || 100;
    ...
    animateOverlappingImages() {
      const intensity = this.container.getAttribute(
        attributes$r.parallaxIntensity
      );
      const offsetRange = 0.25 * intensity;
      const offsetRangeHalf = offsetRange / 2;
      const singleElement = this.container.hasAttribute(
        attributes$r.singleElement
      );
    ...
    animateFooter() {
      ...
      const percentageOfFullOffset = Number(
        (offsetMove / this.footerOffset) * 100
      ).toFixed(2);
      const currentTransform = new WebKitCSSMatrix(
        window.getComputedStyle(this.container).transform
      );
  ...
  class AnnouncementBar {
      ...
      for (let i = 0; i < this.slides.length; i++) {
        ...
        if (
          this.locationPath.indexOf(
            element.getAttribute(attributes$q.targetReferrer)
          ) === -1 &&
    ...
    checkSlidesClasses(slides, barHolder) {
      if (slides.length !== 0) {
        const slidesMobile = barHolder.querySelectorAll(
          `${selectors$C.slide}.${classes$u.mobile}`
        );
        const slidesDesktop = barHolder.querySelectorAll(
          ...
          on: {
            ...
            change: (index) => {
              this.flkty.cells.forEach((slide, i) => {
                slide.element
                  .querySelectorAll(selectors$C.textHighlight)
                  .forEach((highlight) => {
    ...
    toggleTicker(e, isStopped) {
      ...
      const element = document.querySelector(
        `[${attributes$q.slide}="${e.detail.blockId}"]`
      );

      if (isStopped && element) {
        tickerScale.setAttribute(attributes$q.stop, "");
        tickerScale
          .querySelectorAll(selectors$C.tickerText)
          .forEach((textHolder) => {
      ...
      if (!isStopped && element) {
        ...
        if (this.static) {
          document.dispatchEvent(
            new CustomEvent("theme:resize:width", { bubbles: true })
          );
        }
      }
    }
    ...
    updateSVGClipPathIDs() {
      this.barHolder
        .querySelectorAll(selectors$C.slide)
        .forEach((svg, index) => {
  ...
  class HoverDisclosure {
    constructor(el) {
      ...
      this.trigger = document.querySelector(
        `[${attributes$p.disclosureToggle}='${this.key}']`
      );
      ...
      this.dropdown = this.trigger.querySelector(
        selectors$B.disclosureWrappper
      );
      ...
      this.trigger.addEventListener("touchstart", (e) => {
        ...
        if (!isOpen) {
          ...
          const activeNavItems = this.header.querySelectorAll(
            `.${classes$t.isVisible}${selectors$B.navItem}`
          );
  ...
  class Header {
    ...
    handleTouchstart(event) {
      ...
      const activeNavItem = this.header.querySelector(
        `.${classes$s.isVisible}${selectors$A.navItem}`
      );
    ...
    updateBackgroundHeight(event) {
      ...
      let drawersVisible = classes$s.jsDrawerOpenAll.some((popupClass) =>
      ...
      requestAnimationFrame(() => {
        drawersVisible = classes$s.jsDrawerOpenAll.some((popupClass) =>
          document.body.classList.contains(popupClass)
        );
      ...
      this.navSearchOpen?.forEach((element) => {
        element.addEventListener("click", (event) => {
          ...
          const drawer = this.drawerToggle.closest(
            `${selectors$A.drawer}.${classes$s.isOpen}`
          );
          const isMobile = matchMedia("(pointer:coarse)").matches;
          const popdownToggle = isMobile
            ? this.mobileMenu.querySelector(selectors$A.popdownToggle)
            : this.nav.querySelector(selectors$A.popdownToggle);
    ...
    init() {
      if (this.videoPlay.length) {
        this.videoPlay.forEach((element) => {
          element.addEventListener("click", (e) => {
            if (
              element.hasAttribute(attributes$n.videoPlayValue) &&
              element.getAttribute(attributes$n.videoPlayValue).trim() !== ""
            ) {
              e.preventDefault();
  ...
  const selectors$w = {
    ...
    parallax:
      '[data-parallax="zoom-on-scroll"],[data-parallax="card-scrolling"],[data-parallax="zoom-on-scroll,card-scrolling"]',
  };
  ...
  class BannerImage {
    ...
    handleImageAnimation(onLoad = false) {
      ...
      const imageWrapper = this.container.querySelector(
        selectors$w.imageWrapper
      );
      ...
      if (img.length && !imgComplete) {
        ...
        const onImageTransitionEnd = (event) => {
          requestAnimationFrame(() =>
            this.container.classList.add(classes$p.bannerImgLoaded)
          );
  ...
  class ScrollSpy {
    ...
    isEligible() {
      if (this.container.hasAttribute(attributes$m.scrollSpyPrevent))
        return false;

      return (
        (isMobile() && this.anchor.hasAttribute(attributes$m.mobile)) ||
        (isDesktop() && this.anchor.hasAttribute(attributes$m.desktop)) ||
        (!this.anchor.hasAttribute(attributes$m.desktop) &&
          !this.anchor.hasAttribute(attributes$m.mobile))
      );
    }
    ...
    onLoad() {
      ...
      elements.forEach((element) => {
        const scrollSpy = this.container.querySelector(
          element.getAttribute(attributes$m.scrollSpy)
        );
  ...
  class BannerWithTextColumns {
      ...
      this.links.forEach((link) => {
        link.addEventListener("focus", () => {
          const selectedIndex = Number(
            link.closest(selectors$u.banner).getAttribute(attributes$l.index)
          );
    ...
    onBlockSelect(event) {
      ...
      if (this.appearance === settings$2.row) {
        ...
        setTimeout(
          () => this.container.removeAttribute(attributes$l.scrollSpyPrevent),
          1000
        );
      }
    }
  ...
  class ColumnsWithImage {
    ...
    checkSlidesSize() {
      const sliderItemStyle =
        this.container.querySelector(selectors$t.sliderItem).currentStyle ||
        window.getComputedStyle(
          this.container.querySelector(selectors$t.sliderItem)
        );
  ...
  class ContactForm {
    constructor(section) {
      ...
      this.closeButton = this.container.querySelector(
        selectors$s.formMessageClose
      );
      this.messageWrapper = this.container.querySelector(
        selectors$s.formMessageWrapper
      );
  ...
  class CountdownTimer extends HTMLElement {
    constructor() {
      ...
      this.expirationBehavior = this.getAttribute(
        attributes$j.expirationBehavior
      );
  ...
  class VideoBackground {
    constructor(container) {
      ...
      this.videoTemplate = this.container.querySelector(
        selectors$q.videoTemplate
      );
      this.videoPlayButton = this.container.querySelector(
        selectors$q.videoPlayButton
      );
      this.init();
    }
  ...
  class Newsletter {
    ...
    writeStorage() {
      if (this.sessionStorage !== undefined) {
        this.sessionStorage.setItem(
          attributes$i.storageNewsletterFormId,
          this.newsletter.id
        );
      }
    }
            ...
            on: {
              ...
              change: (index) => {
                this.flkty[sliderId].cells.forEach((slide, i) => {
                  slide.element
                    .querySelectorAll(selectors$o.links)
                    .forEach((link) => {
                      link.setAttribute(
  ...
  class GridSlider {
          ...
          if (promos.length && isDesktop) {
            promos.forEach((promo) => {
              if (promo.classList.contains(classes$h.promoFullWidth)) {
                ...
                promo.classList.contains(classes$h.promoTwoItemsWidth)
              ) {
                itemsCount += 1;
              }
            });
    ...
    getTallestProductGridItem(slider) {
      ...
      if (promos.length) {
        const productGridItems = slider.querySelectorAll(
          selectors$n.productGridItems
        );
        const tallestGridItemHeight = Math.max(
          ...Array.from(productGridItems).map(
            (productGridItem) => productGridItem.offsetHeight
          )
        );

        slider.style.setProperty(
          "--carousel-promo-height",
          `${tallestGridItemHeight}px`
        );
      }
    }
  ...
  class Footer {
    constructor(section) {
      ...
      requestIdleCallback(() => {
        if (
          this.container.hasAttribute(attributes$f.parallax) &&
          !this.main.classList.contains(classes$g.mainParallax)
        ) {
          this.init();
          document.addEventListener("theme:resize", this.resizeEvent);
    ...
    init() {
      const isSafari =
        ...
        this.container.hasAttribute(attributes$f.parallaxSafari);
      const stopParallax =
        this.isViewportSmaller() ||
        window.innerWidth < theme.sizes.small ||
        isSafari;
      const body = document.body;
      body.classList.remove(classes$g.bodyRoundedCornersLarge);

      const bgColor = getComputedStyle(this.container).getPropertyValue(
        attributes$f.bg
      ); // get background color of footer section
      this.parentContainer.style.setProperty(
        "--footer-parallax-height",
        `auto`
      ); // reset parallax footer height

      let prevSection = this.parentContainer.previousElementSibling;

      if (prevSection) {
        if (
          !prevSection.classList.contains(classes$g.shopifySection) &&
          !prevSection.classList.contains(classes$g.mainContent)
        ) {
          // if previous section is not a shopify section/main so it should be the main container
        ...
        if (this.wavyEnabled && !stopParallax) {
          ...
          if (
            this.container.hasAttribute(attributes$f.parallaxRounded) &&
            !this.wavyEnabled
          ) {
            ...
            if (
              this.container.hasAttribute(attributes$f.parallaxRoundedLarge)
            ) {
    ...
    onUnload(reset = false) {
      ...
      if (this.footerParallax || reset) {
        setTimeout(() => {
          ...
          const parallaxFooters = document.querySelectorAll(
            selectors$l.footerParallax
          );
          const footerInstance = shopifyInstances.filter(
            (instance) => instance.container === parallaxFooters[0]
          );
  ...
  class Look {
    ...
    dotPointerIn(event) {
      ...
      const image = this.slides[dotIndex].querySelector(
        selectors$h.productGridItemImage
      );
        ...
        if (pswpElement.classList.contains(classes$e.popupOpen)) {
          ...
          const holder =
            this.quickViewPopup.loadPhotoswipe.pswpElement.querySelector(
              `[${attributes$e.hotspot}="${event.target.getAttribute(
                attributes$e.hotspot
              )}"]`
            );
          const quickViewItemHolders =
  ...
  class Mosaic {
    constructor(section) {
      this.container = section.container;
      this.hasSliderMobile = this.container.hasAttribute(
        attributes$d.sliderMobile
      );
      this.carouselMobile = this.container.querySelector(
        selectors$g.carouselMobile
      );
    ...
    updateAOSAnchors() {
      ...
      if (
        isMobile() &&
        this.hasSliderMobile &&
        this.carouselMobile &&
        this.items.length > 1
      ) {
        const firstItem = this.items[0];
        const firstContentInner = firstItem.querySelector(
          selectors$g.itemContentInner
        );
        const blockId = firstItem.id;
        const contentInnerId = firstContentInner
          ? firstContentInner.id
          : blockId;

        firstItem.setAttribute(attributes$d.aosTrigger, `#${blockId}`);
        firstContentInner?.setAttribute(
          attributes$d.aosTrigger,
          `#${contentInnerId}`
        );

        this.items.forEach((item) => {
          const animatable = item.querySelectorAll(selectors$g.animatable);
          const itemContentInner = item.querySelector(
            selectors$g.itemContentInner
          );

          item.setAttribute(attributes$d.aosAnchor, `#${blockId}`);
          itemContentInner?.setAttribute(
            attributes$d.aosAnchor,
            `#${contentInnerId}`
          );
          animatable.forEach((element) =>
            element.setAttribute(attributes$d.aosAnchor, `#${contentInnerId}`)
          );
        });
      }
    }
    ...
    triggerAOS(target) {
      ...
      const isAnimated = [...animatable].some((element) =>
        element.classList.contains(classes$d.aosAnimate)
      );
  ...
  class TargetReferrer {
    constructor(el) {
      ...
      if (
        this.locationPath.indexOf(
          this.popupContainer.getAttribute(attributes$c.targetReferrer)
        ) === -1 &&
        !window.Shopify.designMode
      ) {
  ...
  class LargePopup {
    ...
    hidePopup() {
      ...
      if (
        activePopups == 1 &&
        document.body.classList.contains(classes$c.barIsVisible)
      ) {
        this.scrollUnlock();
    ...
    onBlockSelect(evt) {
      if (
        ...
        !this.popup.classList.contains(classes$c.visible)
      ) {
            ...
            slides.forEach((slide) => {
              ...
              slide.addEventListener("keyup", (event) => {
                if (
                  ...
                  const selectedIndex = Number(
                    slide.getAttribute(attributes$b.logoSlide)
                  );
  ...
  class ProductSticky {
    constructor(section) {
      this.container = section.container;
      this.stickyEnabled =
        this.container.getAttribute(attributes$a.stickyEnabled) === "true";
    ...
    calculateStickyPosition(e = null) {
      ...
      if (this.stickyFormLoad) {
        if (
          document.querySelector(selectors$c.headerSticky) &&
          document.querySelector(selectors$c.headerHeight)
        ) {
          this.stickyDefaultTop = parseInt(
            document
              .querySelector(selectors$c.headerHeight)
  ...
  class Zoom {
    constructor(container) {
      ...
      this.mediaContainer = this.container.querySelector(
        selectors$b.mediaContainer
      );
      this.slider = this.container.querySelector(
        selectors$b.productMediaSlider
      );
      ...
      if (this.zoomWrappers.length) {
        this.zoomWrappers.forEach((element, i) => {
          element.addEventListener("click", (e) => {
            ...
            const isMoving =
              this.slider && this.slider.classList.contains(classes$a.isMoving);
    ...
    createZoom(indexImage) {
      ...
      this.zoomWrappers.forEach((elementImage) => {
        ...
        const imgWidth = parseInt(
          elementImage.getAttribute(attributes$9.dataImageWidth)
        );
        const imgHeight = parseInt(
          elementImage.getAttribute(attributes$9.dataImageHeight)
        );
          ...
          fetch(this.getAttribute(attributes$8.url))
            ...
            .then((text) => {
              ...
              const recommendations = html.querySelector(
                selectors$a.complementaryProducts
              );
  ...
  class Product {
    constructor(section) {
      ...
      if (
        this.container.classList.contains(classes$8.featuredProductOnboarding)
      ) {
        return;
      }
    ...
    initProductSlider() {
      const slider = this.container.querySelector(
        selectors$8.productMediaSlider
      );
      const thumbs = this.container.querySelector(
        selectors$8.productMediaThumbs
      );
      const media = this.container.querySelectorAll(
        selectors$8.productMediaWrapper
      );
      ...
      if (media.length > 1) {
        ...
        if (media.length) {
          media.forEach((el) => {
            el.addEventListener("theme:media:play", () => {
              ...
              el.closest(selectors$8.productMediaSlider).classList.add(
                classes$8.hasMediaActive
              );
            });
            ...
            el.addEventListener("theme:media:pause", () => {
              ...
              this.flkty.updateDraggable();
              el.closest(selectors$8.productMediaSlider).classList.remove(
                classes$8.hasMediaActive
              );
            });
          });
        ...
        if (thumbs !== null) {
          ...
          const thumbLinks = this.container.querySelectorAll(
            selectors$8.productSingleThumbnailLink
          );
    ...
    initUpsellSlider() {
      const slider = this.container.querySelector(
        selectors$8.upsellProductSlider
      );
    ...
    switchMedia(mediaId) {
      ...
      const selectedMedia = this.container.querySelector(
        `${selectors$8.productMediaWrapper}[${attributes$7.mediaId}="${mediaId}"]`
      );
      const isFocusEnabled = !document.body.classList.contains(
        classes$8.noOutline
      );
      ...
      selectedMedia
        .closest(selectors$8.productMediaSlider)
        .classList.remove(classes$8.hasMediaActive);
      ...
      if (
        deferredMedia &&
        deferredMedia.getAttribute(attributes$7.loaded) !== "true"
      ) {
        selectedMedia
          .querySelector(selectors$8.deferredMediaButton)
          .dispatchEvent(new Event("click"));
      }
    }
    ...
    initShopifyXrLaunch() {
      document.addEventListener("shopify_xr_launch", () => {
        const currentMedia = this.container.querySelector(
          `${selectors$8.productModel}:not(.${classes$8.mediaHidden})`
        );
        currentMedia.dispatchEvent(new CustomEvent("xrLaunch"));
      });
    }
    ...
    onBlockSelect(event) {
      const flkty = Flickity.data(
        event.target.closest(selectors$8.featureSlider)
      );
      const index = parseInt(
        [...event.target.parentNode.children].indexOf(event.target)
      );
  ...
  class ProductsList {
        ...
        this.tabButtons.forEach((tabButton) => {
          tabButton.addEventListener("click", (e) => {
            ...
            this.tabItems.forEach((item) => {
              ...
              if (theme.settings.animationsEnabled) {
                item
                  .querySelectorAll(`.${classes$7.aosAnimate}`)
                  .forEach((element) => {
    ...
    initSliders() {
      this.slidersImages.forEach((sliderImages, idx) => {
        const contentsElement = sliderImages
          .closest(selectors$7.tabItem)
          .querySelector(selectors$7.sliderContents);
        ...
        flktyImages.on("settle", (index) => {
          ...
          for (let i = 0; i < elements.length; i++) {
            if (i === index) {
              elements[i]
                .querySelector(selectors$7.productSingleMediaImage)
      ...
      this.slidersContents.forEach((sliderContent) => {
        ...
        flktyContent.on("settle", (index) => {
          const elements = sliderContent.querySelectorAll(
            selectors$7.productContentWrapper
          );

          for (let i = 0; i < elements.length; i++) {
            if (i === index) {
              elements[i]
                .querySelectorAll(selectors$7.productLink)
                .forEach((element) => {
    ...
    switchMedia(mediaId) {
      const selectedMedia = this.container.querySelector(
        `${selectors$7.productMediaWrapper}[${attributes$6.mediaId}="${mediaId}"]`
      );
      const isFocusEnabled = !document.body.classList.contains(
        classes$7.noOutline
      );
    ...
    initShopifyXrLaunch() {
      document.addEventListener("shopify_xr_launch", () => {
        const currentMedia = this.container.querySelector(
          `${selectors$7.productModel}:not(.${classes$7.mediaHidden})`
        );
        currentMedia.dispatchEvent(new CustomEvent("xrLaunch"));
      });
    }
  ...
  class RelatedProducts {
    ...
    recent() {
      const recentlyViewed = this.container.querySelector(
        selectors$6.recentlyViewed
      );
      ...
      Shopify.Products.showRecentlyViewed({
        ...
        onComplete: (wrapper, section) => {
          ...
          const recentlyViewedHolder = container.querySelector(
            selectors$6.recentlyViewed
          );
          const recentlyViewedWrapper = container.querySelector(
            selectors$6.recentlyViewedWrapper
          );
          const recentProducts = wrapper.querySelectorAll(selectors$6.product);
          const aosItem = recentlyViewedHolder.querySelectorAll(
            selectors$6.aos
          );
          ...
          if (checkRecentInRelated || checkRecentOutsideRelated) {
            ...
            if (slider) {
              ...
              requestAnimationFrame(() => {
                this.recentGridSlider.sliders.forEach((slider) => {
                  slider.dispatchEvent(
                    new CustomEvent("theme:slider:resize", { bubbles: true })
                  );
            ...
            let itemsSet = [];
            recentlyViewedHolder.addEventListener(
              "theme:target:animate",
              (event) => {
                itemsSet.push(event.detail);
                if (itemsSet.length === aosItem.length) {
                  requestAnimationFrame(() =>
                    aosItem.forEach((item) =>
                      item.classList.add(classes$6.aosAnimate)
                    )
                  );
                }
              }
    ...
    hideRelated() {
      ...
      const tabsLink = tabsHolder.querySelector(
        `[${attributes$5.tabsLink}="${tab.dataset.tab}"]`
      );
      ...
      tabsLink.remove();
      tabsHolder
        .querySelector(selectors$6.tabsLink)
        .dispatchEvent(new Event("click"));
  ...
  class ShoppableBlog {
    constructor(section) {
      ...
      this.clickOutsideItemEvent = (e) => {
        const clickOutsideSliderItem = !(
          e.target.matches(selectors$5.sliderItem) ||
          e.target.closest(selectors$5.sliderItem)
        );

        if (clickOutsideSliderItem) {
          const sliderItem = this.container.querySelectorAll(
            selectors$5.sliderItem
          );
      ...
      this.flkty.on("change", () => {
        ...
        if (slides.length) {
          slides.forEach((el) => {
            ...
            if (itemProducts) {
              el.querySelector(selectors$5.itemProducts).classList.remove(
                classes$5.itemProductsVisible
              );
            }
          });
        }
    ...
    checkSlidesSize() {
      const sliderItemStyle =
        this.container.querySelector(selectors$5.sliderItem).currentStyle ||
        window.getComputedStyle(
          this.container.querySelector(selectors$5.sliderItem)
        );
      this.gutter = parseInt(sliderItemStyle.marginRight);
      ...
      if (buttonProductsShow.length) {
        buttonProductsShow.forEach((button) => {
          button.addEventListener("click", (e) => {
            ...
            this.container
              .querySelectorAll(selectors$5.sliderItem)
              .forEach((item) => {
                const itemProducts = item.querySelector(
                  selectors$5.itemProducts
                );
                item.classList.remove(classes$5.itemActive);
            ...
            if (itemProducts) {
              ...
              if (sliderActive) {
                ...
                const currentSlideIndex = currentSlide.getAttribute(
                  attributes$4.slidePosition
                );

                allSlides.forEach((slide, i) => {
                  slide.setAttribute(
                    attributes$4.tabIndex,
              ...
              on: {
                change: (index) => {
                  flktyProducts.cells.forEach((slide, i) => {
                    slide.element
                      .querySelectorAll(selectors$5.links)
                      .forEach((link) => {
                        link.setAttribute(
                          attributes$4.tabIndex,
                          i === index ? "0" : "-1"
  ...
  class Slider {
    ...
    initSlider() {
      ...
      if (slidesCount > 1) {
        this.flkty = new Flickity(this.container, {
          ...
          on: {
            ready: () => {
              const currentStyle = this.items[0].getAttribute(
                attributes$3.style
              );
              this.container.setAttribute(
                attributes$3.currentStyle,
                currentStyle
              );
            ...
            change: (index) => {
              ...
              this.flkty.cells.forEach((slide, i) => {
                slide.element
                  .querySelectorAll(selectors$4.links)
                  .forEach((link) => {
                    link.setAttribute(
                      attributes$3.tabIndex,
                      i === index ? "0" : "-1"
                    );
                  });

                slide.element
                  .querySelectorAll(selectors$4.textHighlight)
                  .forEach((highlight) => {
                    highlight.setTriggerAttribute(Boolean(i === index));
                  });
              });
    ...
    getTallestSlideItem() {
      ...
      const isOriginalHeight = [...this.items].some((item) =>
        item.classList.contains(classes$4.originalHeight)
      );
      const isOriginalHeightMobile = [...this.items].some((item) =>
        item.classList.contains(classes$4.originalHeightMobile)
      );
      const hasMobileHeight = this.container.classList.contains(
        classes$4.hasMobileHeight
      );

      if (
        (isDesktop() && !isOriginalHeight) ||
        (isMobile() && !isOriginalHeightMobile && hasMobileHeight) ||
        (isMobile() && !isOriginalHeight && !hasMobileHeight)
      ) {
        const heightDiffs = [...this.items].some(
          (item) => Math.floor(item.offsetHeight) !== tallestSlideItemHeight
        );

        if (heightDiffs) {
          this.container.style.setProperty(
            "--item-min-h",
            `${tallestSlideItemHeight}px`
          );
        }
      }
    }
    ...
    handleFirstSlideAnimation(onLoad = false) {
      ...
      const slideImgComplete = this.container.classList.contains(
        classes$4.imgIn
      );
      ...
      if (slideImage.length && !slideImgComplete) {
        ...
        const onImageTransitionEnd = (event) => {
          requestAnimationFrame(() =>
            this.container.classList.add(classes$4.sliderImgLoaded)
          );
          slideImage[0].removeEventListener(
            "transitionend",
    ...
    onBlockDeselect(evt) {
      const autoplay =
        evt.target.closest(selectors$4.slider).dataset.autoplay === "true";
  ...
  class StickyImageCards {
    constructor(section) {
      ...
      this.layoutMobile = this.container.getAttribute(
        attributes$2.layoutMobile
      );
    ...
    isEligible() {
      ...
      const elementOffsetBottomPoint =
        elementOffsetTopPoint + this.container.offsetHeight;
      const isBottomOfElementPassed =
        elementOffsetBottomPoint < Math.round(window.scrollY);
      const isTopOfElementReached =
        elementOffsetTopPoint < Math.round(window.scrollY + window.innerHeight);
      const isInView = isTopOfElementReached && !isBottomOfElementPassed;
    ...
    detectStickyPositions() {
      const parentOffset = Math.round(
        this.body.getBoundingClientRect().top -
          this.headerHeight -
          this.wrapperTop
      );
      ...
      [...this.wrapper].forEach((item, i) => {
        ...
        if (this.isMobile) {
          if (i !== this.lastIndex) {
            const nextOneDistance = Math.floor(
              this.wrapper[i + 1].getBoundingClientRect().top -
                this.headerHeight
            );
            item.classList.toggle(
              classes$3.hasPassedSticky,
              nextOneDistance <= this.stickyTop
            );
          }

          this.container.classList.toggle(
            `checkpoint-${i + 1}`,
            parentOffset <= this.cumulativeHeights[i] * -1
          );
        }
    ...
    calculateHeights() {
      ...
      this.spacingTop =
        parseInt(
          window.getComputedStyle(this.container).getPropertyValue("--padding")
        ) || 20;
      this.wrapperTop =
        parseInt(
          window.getComputedStyle(this.wrapper[0]).getPropertyValue("top")
        ) || 0;
      this.stickyTop = Math.floor(this.wrapperTop - this.headerHeight);
      this.gap =
        parseInt(window.getComputedStyle(this.aside).getPropertyValue("gap")) ||
        0;
      ...
      [...this.wrapper].forEach((item, i) => {
        ...
        requestAnimationFrame(() => {
          ...
          const isExceeding =
            itemHeight + this.wrapperTop + this.headerHeight >
            this.viewportHeight;

          const heights = [...this.wrapper].map((element) =>
            Number(element.dataset.index) >= i ? 0 : element.offsetHeight
          );
    ...
    onBlockSelect(event) {
      requestAnimationFrame(() => {
        ...
        const parentOffset = Math.round(
          this.body.getBoundingClientRect().top + window.scrollY
        );

        // Sum the heights of each previous sibling container
        const heights = [...this.wrapper].map((element) =>
          Number(element.dataset.index) >= index ? 0 : element.offsetHeight
        );
        ...
        const distance =
          parentOffset + cumulativeHeight - adjustTop() - this.headerHeight;
  ...
  class StickyImagesAndText {
    constructor(section) {
      ...
      this.horizontalScroll =
        this.scrollDirection === settings.scrollDirection.horizontal ||
        this.scrollDirection === settings.scrollDirection.horizontalReversed;
      this.verticalScroll =
        this.scrollDirection === settings.scrollDirection.vertical ||
        this.scrollDirection === settings.scrollDirection.verticalReversed;
      ...
      this.textHighlight = this.container.querySelectorAll(
        selectors$2.textHighlight
      );
    ...
    init() {
      ...
      this.calculateHeights();
      this.performAnimation = requestAnimationFrame(
        this.calculateImagesOffsetCallback
      );
      requestAnimationFrame(() => {
      ...
      this.observer = new IntersectionObserver(
        (entries, observer) => {
          entries.forEach((entry) => {
            ...
            const checkMaxVisibility =
              this.typeOfIntersecting ===
              settings.typeOfIntersecting.maxVisibility;
            ...
            if (checkMaxVisibility && isIntersecting) {
              ...
              this.compareIntersectionRatio = [...this.observersCollection].map(
                (item) => item.intersectionRatio
              );
            ...
            if (isIntersecting && intersectionRatio > intersectAt) {
              if (
                checkMaxVisibility &&
                this.maxVisibility === intersectionRatio
              ) {
                this.maxVisibilityItem = [...this.observersCollection].find(
                  (item) => item.intersectionRatio === this.maxVisibility
                );
                this.activeIndex = Number(
                  this.maxVisibilityItem.target.dataset.index
                );
        ...
        },
        {
          ...
          rootMargin: "0px",
          threshold: [
            0, 0.15, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.85, 1,
          ],
        }
      );
    ...
    checkImagePositionInViewport(element) {
      ...
      if (
        scrollMiddle > elementOffsetTopPoint &&
        elementOffsetBottomPoint > scrollMiddle
      ) {
    ...
    calculateImagesOffset() {
      const asideTop =
        this.aside.getBoundingClientRect().top - this.headerHeight;
      ...
      const triggerPoint = !this.hasShortImages
        ? asideTop
        : stickyStartingPoint;
      ...
      if (isMobile() || this.horizontalScroll) {
        this.aside.style.setProperty(
          "--translateX",
          `${Number(offset * this.direction).toFixed(2)}%`
        );
      }
    ...
    calculateHeights() {
      ...
      if (this.text.length > 0) {
        const contentPaddingTop = this.content
          ? parseInt(
              window
          ...
          : 0;
        const contentPaddingBottom = this.content
          ? parseInt(
              window
                .getComputedStyle(this.content)
          ...
          : 0;
        const textElementsHeights = [...this.text].map(
          (element) =>
            element.offsetHeight + contentPaddingTop + contentPaddingBottom
        );
        const textInnerElementsHeights = [...this.textInner].map(
        ...
        const textElementsHeightsSum = textElementsHeights.reduce(
          (a, b) => a + b,
          0
        );
        const averageTextHeight =
          Math.floor(textElementsHeightsSum / textElementsHeights.length) ||
          this.text[0].offsetHeight;

        this.container.style.removeProperty("--average-text-height");
        this.container.style.setProperty(
          "--average-text-height",
          `${averageTextHeight}px`
        );
      ...
      if (this.image.length > 0) {
        this.imgHeight = this.image[0].querySelector(
          selectors$2.imageWrapper
        ).offsetHeight;
        this.imgPadding = parseInt(
          window.getComputedStyle(this.image[0]).getPropertyValue("padding-top")
        );

        this.container.style.removeProperty("--images-height");
        this.container.style.setProperty(
          "--images-height",
          `${this.imagesHeight}px`
        );
        this.container.style.removeProperty("--img-height");
        this.container.style.setProperty(
          "--img-height",
          `${this.imgHeight + 2 * this.imgPadding}px`
        );
      }

      // Variables used in `calculateImagesOffset()` method
      this.asideHeight = this.aside.offsetHeight;
      this.asidePadding = parseInt(
        window.getComputedStyle(this.aside).getPropertyValue("padding-top")
      );
      this.breathingSpace = (this.windowHeight - this.imagesHeight) / 2 / 2;
      this.innerStickyTop =
        parseInt(window.getComputedStyle(this.inner).getPropertyValue("top")) -
        this.headerHeight -
        this.breathingSpace;
      this.gutters = this.withGaps ? 2 * this.asidePadding : 0;
      this.additionalSpace = isMobile()
        ? this.withGaps
          ? this.asidePadding + 10
          : 10
        : this.gutters;
      ...
      if (!isMobile() && this.horizontalScroll) {
        ...
        if (this.hasShortImages) {
          this.minHeightForEndingPoing =
            this.imagesHeight - this.innerStickyTop + this.breathingSpace * 3;
        }
      }
    ...
    sync() {
      ...
      [...this.image, ...textsToSync, ...this.dot].forEach((element) => {
        ...
        element.classList.toggle(
          classes$2.isActive,
          elementIndex === Number(this.activeIndex)
        );
      });
    ...
    syncDots() {
      this.dot.forEach((element) => {
        element.addEventListener("click", (event) => {
          ...
          const targetOffsetTop = Math.round(
            imageToScrollTo.getBoundingClientRect().top + window.scrollY
          );
          ...
          if (this.verticalScroll && isDesktop()) {
            ...
            if (
              this.typeOfIntersecting ===
              settings.typeOfIntersecting.middleOfViewport
            ) {
              distance =
                targetOffsetTop -
    ...
    onBlockSelect(event) {
      ...
      const imagesHeight = this.images.offsetHeight;
      const scrollOffset = containerTop + imagesHeight * activeIndex;
      const additionalSpace = isMobile()
        ? Math.round(10 / (this.image.length - 1)) * activeIndex
        : 0;
  ...
  class Testimonials {
    ...
    onResize() {
      ...
      if (this.flkty !== null) {
        ...
        if (this.flkty.size.width >= this.flkty.slideableWidth)
          this.destroySlider();
      }
    }
    ...
    checkSlideCount() {
      ...
      const twoSlidesOnTablet = Boolean(
        this.slidesCount == 2 && window.innerWidth >= theme.sizes.small
      );
      ...
      this.flkty = new Flickity(this.slider, {
        ...
        on: {
          ready: () => {
            requestAnimationFrame(() => {
              this.slider
                .querySelectorAll(selectors$1.button)
                .forEach((button) => {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (
      ...
      window.theme.settings.colorSwatchesType != "disabled"
    ) {
